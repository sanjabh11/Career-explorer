 ...
 */
async function testWithCurl(apiName, url, headers) {
  try {
...
async function main() {
  ...
  if (!jinaApiKey) {
    ...
    console.log('✅ REACT_APP_JINA_API_KEY is set');
  }
  
  if (!serpApiKey) {
    console.error('❌ REACT_APP_SERP_API_KEY is not set in .env file');
  } else {
    console.log('✅ REACT_APP_SERP_API_KEY is set');
  }
  ...
  try {
    ...
    if (jinaApiKey) {
      ...
      jinaSuccess = jinaResult.success;
      console.log(`\nJina API Curl Test: ${jinaSuccess ? '✅ Success' : '❌ Failed'}`);
    ...
    if (serpApiKey) {
      ...
      serpSuccess = serpResult.success;
      console.log(`\nSERP API Curl Test: ${serpSuccess ? '✅ Success' : '❌ Failed'}`);
    ...
    console.log('\n📋 Summary:');
    console.log('===========');
    console.log(`Jina API: ${jinaSuccess ? '✅ Success' : '❌ Failed'}`);
    console.log(`SERP API: ${serpSuccess ? '✅ Success' : '❌ Failed'}`);
    
    const allSuccess = jinaSuccess && serpSuccess;
