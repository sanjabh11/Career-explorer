...
const TechnologyTrends: React.FC<TechnologyTrendsProps> = ({
  ...
  const [trendData, setTrendData] = useState<TechnologyTrendData[]>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const data = await getTechnologyTrends(occupationCode, industryCode);
        setTrendData(data);
        setLoading(false);
      } catch (err) {
  ...
  const getTrendIcon = (trend: TechnologyTrend) => {
    switch (trend) {
      case TechnologyTrend.Emerging:
      case TechnologyTrend.Growing:
        return <TrendingUpIcon color="success" />;
      case TechnologyTrend.Stable:
        return <TrendingFlatIcon color="action" />;
      case TechnologyTrend.Declining:
  ...
  const getTrendColor = (trend: TechnologyTrend): string => {
    switch (trend) {
      case TechnologyTrend.Emerging:
        return '#4caf50'; // Green
      case TechnologyTrend.Growing:
  ...
  const displayTrends = trendData.slice(0, limit);
              ...
              >
                ...
                <TableCell>
                  <Chip
                    ...
                    size="small"
                    sx={{ 
                      bgcolor: `${getTrendColor(trend.trend)}20`, 
                ...
                <TableCell>
                  <Box sx={{ display: 'flex', alignItems: 'center' }}>
                    <Box sx={{ width: '100%', mr: 1 }}>
                      <LinearProgress 
                        variant="determinate" 
                        value={trend.currentAdoption * 100}
                        sx={{ height: 8, borderRadius: 4 }}
                      />
                    </Box>
                    <Box sx={{ minWidth: 35 }}>
                      <Typography variant="body2" color="text.secondary">
                        {Math.round(trend.currentAdoption * 100)}%
                      </Typography>
                    </Box>
                  </Box>
                </TableCell>
