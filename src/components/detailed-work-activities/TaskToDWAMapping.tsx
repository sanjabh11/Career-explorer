...
const TaskToDWAMapping: React.FC<TaskToDWAMappingProps> = ({
  ...
  const matrixData = useMemo(() => {
    ...
    const tasks = [...mapping.tasks].sort((a, b) => b.importance - a.importance);
  ...
  const groupedByTask = useMemo(() => {
    ...
    for (const task of mapping.tasks) {
      const connectedActivityIds = mapping.connections
        .filter(conn => conn.taskId === task.id)
        .map(conn => conn.activityId);
      
      const connectedActivities = mapping.activities
        .filter(activity => connectedActivityIds.includes(activity.id))
        .sort((a, b) => {
          const aImportance = a.importance || 0;
          const bImportance = b.importance || 0;
          return bImportance - aImportance;
  ...
  const groupedByActivity = useMemo(() => {
    ...
    for (const activity of mapping.activities) {
      const connectedTaskIds = mapping.connections
        .filter(conn => conn.activityId === activity.id)
        .map(conn => conn.taskId);
      
      const connectedTasks = mapping.tasks
        .filter(task => connectedTaskIds.includes(task.id))
              ...
              <TableBody>
                {matrixData.tasks.map((task) => (
                  <TableRow key={task.id} hover>
                    <TableCell sx={{ p: 1 }}>
                      <Tooltip title={`Importance: ${task.importance.toFixed(1)}/5`} arrow>
                    ...
                    {matrixData.activities.map((activity) => {
                      const connection = matrixData.connections.find(
                        conn => conn.taskId === task.id && conn.activityId === activity.id
                      );
      ...
      <Box>
        ...
        <List>
          {mapping.connections.map((connection, index) => {
            const task = mapping.tasks.find(t => t.id === connection.taskId);
            const activity = mapping.activities.find(a => a.id === connection.activityId);
                  ...
                  <Grid container spacing={2}>
                    <Grid item xs={5}>
                      ...
                      <Typography variant="caption" color="text.secondary">
                        Importance: {task.importance.toFixed(1)}/5
                      </Typography>
                    </Grid>
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', width: '100%' }}>
                  ...
                  <Box sx={{ flex: 1 }}>
                    ...
                    <Box sx={{ display: 'flex', alignItems: 'center', mt: 0.5 }}>
                      <Chip 
                        label={`Importance: ${task.importance.toFixed(1)}/5`} 
                        size="small" 
                        variant="outlined"
                        sx={{ mr: 1 }}
                      />
                    ...
                    {relatedTasks.map((task) => (
                      <ListItem key={task.id}>
                        <ListItemText
                          ...
                          secondary={`Importance: ${task.importance.toFixed(1)}/5`}
                        />
                      </ListItem>
                    ))}
                  </List>
