    ...
    const fetchData = async () => {
      try {
        ...
        if (targetOccupationCode && compareMode) {
          data = await compareSkillsBetweenOccupations(sourceOccupationCode, targetOccupationCode);
        } else {
          ...
          <Grid item xs={12} sm={6}>
            <Box display="flex" alignItems="center">
              ...
              <Typography variant="subtitle2">
                {transferabilityData.sourceOccupation.title} ({transferabilityData.sourceOccupation.code})
              </Typography>
            </Box>
          </Grid>
          ...
          {compareMode && targetOccupationCode && transferabilityData.targetOccupation && (
            <Grid item xs={12} sm={6}>
              <Box display="flex" alignItems="center">
                ...
                <Typography variant="subtitle2">
                  {transferabilityData.targetOccupation.title} ({transferabilityData.targetOccupation.code})
                </Typography>
              </Box>
            </Grid>
          )}
      ...
      {selectedNode && (
        <Box mt={3}>
          ...
          <Card variant="outlined">
            <CardContent>
              {transferabilityData.nodes.filter(node => node.id === selectedNode).map(node => (
                <Box key={node.id}>
                  <Typography variant="h6">
                    {node.label}
                  </Typography>
                  ...
                  <Grid container spacing={1}>
                    {transferabilityData.edges
                      ...
                      .map((edge, index) => {
                        ...
                        const connectedNode = transferabilityData.nodes.find(n => n.id === connectedNodeId);
